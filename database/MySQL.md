...
> 练习题 SQL 见同目录：https://github.com/zljin/cheatsheet/tree/master/database

### 1. 建表 DDL

```sql
-- char(10) 固定占10个字符空间；varchar(255) 可变，最多255个字符
-- bigint(10) 仅开启 ZEROFILL 后前面补0，否则括号内数字不影响存储
CREATE TABLE `t1` (
...
```sql
-- 子查询（效率一般）：in 在内存中遍历比较，类似双重循环，对内存需求高
SELECT name FROM users WHERE id IN (SELECT user_id FROM orders WHERE amount > 100);

-- EXISTS（常用于判断存在性）：外层循环 + 每次内层查库按索引匹配
...
```sql
-- 按 分组字段 分组、内部按 排序字段 排序后编序号
ROW_NUMBER()  OVER (PARTITION BY 分组字段 ORDER BY 排序字段)  -- 1,2,3...连续，无并列
RANK()        OVER (PARTITION BY 分组字段 ORDER BY 排序字段)  -- 有并列，并列后跳号（1,1,3）
DENSE_RANK()  OVER (PARTITION BY 分组字段 ORDER BY 排序字段)  -- 有并列，并列后不跳号（1,1,2）
SUM(...)      OVER (PARTITION BY 分组字段 ORDER BY 排序字段)  -- 累计求和（跑数）
```
